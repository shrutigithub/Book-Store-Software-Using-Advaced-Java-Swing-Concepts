...
import java.sql.*;
import java.io.*;
class qw7 extends JFrame implements ActionListener 
{
...
jf2.setBackground(Color.CYAN);
		...
		if(s.equals("Pay"))
	{
		 try
       {
		...
		String cadd=tadd.getText();
		int bkct=c;
         Class.forName("oracle.jdbc.driver.OracleDriver");
         Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:XE","system","960909");
		System.out.println("Pament button connected");
      
         
      PreparedStatement p=con.prepareStatement("Insert into client values(?,?,?,?)");
			p.setString(1,cname);
			p.setString(2,bname);
			p.setInt(3,bkct);
			p.setString(4,cadd);
